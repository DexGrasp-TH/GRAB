...
class GRABDataSet(object):
    ...
    def data_preprocessing(self, cfg):
        ...
        for split in self.split_seqs.keys():
            ...
            for sequence in tqdm(self.split_seqs[split], desc=f"Processing {split} sequences"):
                ...
                # # DEBUG
                # if not ("mug_lift" in sequence and "s8" in sequence):
                #     continue
                ...
                if cfg.save_lhand_verts:
                    ...
                    current_lhand_data["verts"] = to_cpu(lh_output.vertices)
                    current_lhand_data["joints"] = to_cpu(lh_output.joints)
                ...
                if cfg.save_rhand_verts:
                    ...
                    current_rhand_data["verts"] = to_cpu(rh_output.vertices)
                    current_rhand_data["joints"] = to_cpu(rh_output.joints)
                ...
                for segment_idx, (start, end) in enumerate(zip(starts, ends)):
                    ...
                    local_max_contact_frame_id = np.argmax(segment_contacts)

                    ############# Select the grasp frame #############
                    # Select the grasp frame as the frame just before the object moving
                    obj_transl = current_object_data["transl"][start:end]
                    obj_dist_to_init = np.linalg.norm(obj_transl - obj_transl[0, :], axis=-1)

                    moving_indices = np.where(obj_dist_to_init > OBJ_MOVE_THRES)[0]
                    if moving_indices.size > 0:
                        # If motion is detected, take the frame immediately preceding it.
                        # Use max(0, ...) to prevent -1 index if it moves at frame 0.
                        local_grasp_frame_id = max(0, moving_indices[0] - 1)
                    else:
                        # Fallback: Maximum contact points
                        local_grasp_frame_id = local_max_contact_frame_id

                    grasp_frame_id = start + local_grasp_frame_id  # to get the pose info
                    # max_contact_frame_id = start + local_max_contact_frame_id  # to get the contact info

                    # Compute bool contacts of each fingertip from the max_contact_frame_id
                    contacts = filtered_contact_data[grasp_frame_id]
                    hand_contacts = {"left": [], "right": []}
                    finger_names = ["Thumb", "Index", "Middle", "Ring", "Pinky"]
                    for name in finger_names:
                        hand_contacts["left"].append(
                            any(f"L_{name}" in contact_id_to_name[c] for c in contacts if c > 0)
                        )
                    for name in finger_names:
                        hand_contacts["right"].append(
                            any(f"R_{name}" in contact_id_to_name[c] for c in contacts if c > 0)
                        )

                    ############# Re-format and save the grasp data #############

                    if np.sum(hand_contacts["left"]) + np.sum(hand_contacts["right"]) < 2:
                        if self.verbose:
                            print("Less than 2 contacts. Skip this grasp.")
                        continue
                    ...
                    new_data["object"]["name"] = obj_name
                    new_data["object"]["path"] = os.path.join(self.out_path, "object", obj_name, f"{obj_name}.obj")
                    new_data["object"]["rel_scale"] = 1.0
                    obj_pose = np.eye(4)
                    obj_pose[:3, 3] = current_object_data["transl"][grasp_frame_id]
                    obj_pose[:3, :3] = (
                        sciR.from_rotvec(current_object_data["global_orient"][grasp_frame_id]).as_matrix().T
                    )  # Require this transpose in GRAB dataset

                    new_data["object"]["pose"] = obj_pose  # 4*4 matrix

                    for hand_idx, side in enumerate(["left", "right"]):
                        if not any(hand_contacts[side]):
                            continue
                        current_hand_data = current_lhand_data if side == "left" else current_rhand_data

                        # new_data["hand"][side]["trans"] = current_hand_data["transl"][grasp_frame_id] # not equal to position of joint 0
                        new_data["hand"][side]["trans"] = current_hand_data["joints"][grasp_frame_id][0, :]
                        new_data["hand"][side]["rot"] = current_hand_data["global_orient"][grasp_frame_id]
                        new_data["hand"][side]["mano_pose"] = current_hand_data["hand_pose"][grasp_frame_id]
                        new_data["hand"][side]["mano_betas"] = np.zeros(
                            (1, 10)
                        )  # TODO: does not consider the hand shape (defined by v_template)
                        new_data["hand"][side]["scale"] = 1000.0
                        new_data["hand"][side]["contacts"] = hand_contacts[side]

                    # 保存到硬盘
                    original_seq_path = sequence
                    base_name = os.path.basename(original_seq_path).split(".")[0]
                    save_dir = os.path.join(self.out_path, "grasp", obj_name)
                    os.makedirs(save_dir, exist_ok=True)
                    save_filename = f"{sbj_id}_{base_name}_seg{segment_idx}.npy"
                    save_full_path = os.path.join(save_dir, save_filename)
                    np.save(save_full_path, new_data)
    ...
    def load_obj_verts(self, obj_name, seq_data, n_verts_sample=512):
        ...
        if obj_name not in self.obj_info:
            ...
            # Mingrui: Re-save the object meshes to customized folder
            save_file_path = os.path.join(self.out_path, f"object/{obj_name}/{obj_name}.obj")
            os.makedirs(os.path.dirname(save_file_path), exist_ok=True)
            obj_mesh.export(save_file_path)
            print(f"Successfully converted {obj_name}'s mesh to OBJ. Vertices: {len(obj_mesh.vertices)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--object-set", default="all", type=str, help="The set of objects to process")
    ...
    out_path = "/data/dataset/AnyScaleGrasp/GRAB"
    ...
    GRABDataSet(cfg, logger, verbose=True)
